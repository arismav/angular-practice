...
import { NotificationPayload, NotificationSeverity, NotificationType, WsMessage } from '../../models/ws-message.model';
...
> = {
  info: { icon: 'ℹ', border: 'border-sky-200', bg: 'bg-sky-50', title: 'text-sky-800', badge: 'bg-sky-100 text-sky-700' },
  success: { icon: '✓', border: 'border-emerald-200', bg: 'bg-emerald-50', title: 'text-emerald-800', badge: 'bg-emerald-100 text-emerald-700' },
  warning: { icon: '⚠', border: 'border-amber-200', bg: 'bg-amber-50', title: 'text-amber-800', badge: 'bg-amber-100 text-amber-700' },
  error: { icon: '✕', border: 'border-red-200', bg: 'bg-red-50', title: 'text-red-800', badge: 'bg-red-100 text-red-700' },
};
...
export class WsNotificationsComponent {
  ...
  checkBg(n: WsMessage<NotificationPayload>): {icon: string, border: string, bg: string, title: string, badge: string} {
    if (n.payload.severity === NotificationType.error) { 
      return { icon: '✕', border: 'border-red-200', bg: 'bg-red-50', title: 'text-red-800', badge: 'bg-red-100 text-red-700' }
    }

    if (n.payload.severity === NotificationType.info) { 
      return { icon: 'ℹ', border: 'border-sky-200', bg: 'bg-sky-50', title: 'text-sky-800', badge: 'bg-sky-100 text-sky-700' }
    }

    if (n.payload.severity === NotificationType.success) { 
      return { icon: '✓', border: 'border-emerald-200', bg: 'bg-emerald-50', title: 'text-emerald-800', badge: 'bg-emerald-100 text-emerald-700' }
    }

    if (n.payload.severity === NotificationType.warning) { 
      return { icon: '⚠', border: 'border-amber-200', bg: 'bg-amber-50', title: 'text-amber-800', badge: 'bg-amber-100 text-amber-700' }
    }
